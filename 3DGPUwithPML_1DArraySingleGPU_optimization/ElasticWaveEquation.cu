...
__global__ void velocityUpdate( GRID grid, Delta delta, VelocityStress vsGPU, DifferenceCoefficient dc, Medium medGPU, int pml ) //__global__ 
{
	...
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX0].startX, vsPML[VERTEX0].startY, vsPML[VERTEX0].startZ, vsPML[VERTEX0].endX, vsPML[VERTEX0].endY, vsPML[VERTEX0].endZ ) ) 
	{
		...
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endX - ix, 			pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endY - iy, 			pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endZ - iz, 			pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endX - ix, 			pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endY - iy, 			pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX0].endZ - iz, 			pml, delta.z );

		VelocityPMLUpdate(  VERTEX0, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , / , dampingPlusX, / , dampingPlusY, / , dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX1].endX - ix, 			pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX1].endY - iy, 			pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX1].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX1].endX - ix, 			pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX1].endY - iy, 			pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX1].startZ + 1, 		pml, delta.z );

		VelocityPMLUpdate(  VERTEX1, dampingMinusX, * , dampingMinusY, * , dampingMinusZ,  * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX2].endX - ix, 			pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX2].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX2].endZ - iz, 			pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX2].endX - ix, 			pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX2].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX2].endZ - iz, 			pml, delta.z );

		VelocityPMLUpdate( VERTEX2, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX3].endX - ix, 			pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX3].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX3].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX3].endX - ix, 			pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX3].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX3].startZ + 1, 		pml, delta.z );

		VelocityPMLUpdate( VERTEX3, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX4].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX4].endY - iy, 			pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX4].endZ - iz, 			pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX4].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX4].endY - iy, 			pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX4].endZ - iz, 			pml, delta.z );

		VelocityPMLUpdate( VERTEX4, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX5].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX5].endY - iy, 			pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX5].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX5].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX5].endY - iy, 			pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX5].startZ + 1, 		pml, delta.z );

		VelocityPMLUpdate( VERTEX5, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX6].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX6].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX6].endZ - iz, 			pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX6].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX6].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[VERTEX6].endZ - iz, 			pml, delta.z );

		VelocityPMLUpdate( VERTEX6, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX7].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX7].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX7].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[VERTEX7].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[VERTEX7].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[VERTEX7].startZ + 1, 		pml, delta.z );

		VelocityPMLUpdate( VERTEX7, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE0].endX - ix, 			pml, delta.x );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE0].endX - ix, 			pml, delta.x );
		VelocityPMLUpdate( SURFACE0, dampingMinusX, *,  ,  ,  ,  , /, dampingPlusX,  ,  ,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[SURFACE1].startX + 1, 	pml, delta.x );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[SURFACE1].startX + 1, 	pml, delta.x );
		VelocityPMLUpdate( SURFACE1, dampingMinusX, * ,  ,  ,  ,  , / , dampingPlusX,  ,  ,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE2].endY - iy, 			pml, delta.y );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE2].endY - iy, 			pml, delta.y );
		VelocityPMLUpdate( SURFACE2,  ,  , dampingMinusY, * ,  ,  ,  ,  , / , dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[SURFACE3].startY + 1, 	pml, delta.y );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[SURFACE3].startY + 1, 	pml, delta.y );
		VelocityPMLUpdate( SURFACE3,  ,  , dampingMinusY,  * ,  ,  ,  ,  , /, dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE4].endZ - iz, 			pml, delta.z );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE4].endZ - iz, 			pml, delta.z );
		VelocityPMLUpdate( SURFACE4,  ,  ,  ,  , dampingMinusZ,  * ,  ,  ,  ,  , /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[SURFACE5].startZ + 1, 	pml, delta.z );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[SURFACE5].startZ + 1, 	pml, delta.z );
		VelocityPMLUpdate( SURFACE5,  ,  ,  ,  , dampingMinusZ, * ,  ,  ,  ,  , /,  dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endX - ix, 				pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endY - iy, 				pml, delta.y );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endX - ix, 				pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endY - iy, 				pml, delta.y );
		VelocityPMLUpdate( EDGE0, dampingMinusX, * , dampingMinusY,  * ,  ,  , /, dampingPlusX, /,  dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE1].endX - ix, 				pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE1].startY + 1, 		pml, delta.y );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE1].endX - ix, 				pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE1].startY + 1, 		pml, delta.y );
		VelocityPMLUpdate( EDGE1, dampingMinusX, * , dampingMinusY, * ,  ,  , /,  dampingPlusX, /,  dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endX - ix, 				pml, delta.x );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endZ - iz, 				pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endX - ix, 				pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endZ - iz, 				pml, delta.z );
		VelocityPMLUpdate( EDGE2, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , / , dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE3].endX - ix, 				pml, delta.x );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE3].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE3].endX - ix, 				pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE3].startZ + 1, 		pml, delta.z );
		VelocityPMLUpdate( EDGE3, dampingMinusX,  * ,  ,  , dampingMinusZ,  * , /,  dampingPlusX,  ,  , / ,  dampingPlusZ )
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE4].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE4].endY - iy, 				pml, delta.y );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE4].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE4].endY - iy, 				pml, delta.y );
		VelocityPMLUpdate( EDGE4, dampingMinusX, * , dampingMinusY, * ,  ,  , /,  dampingPlusX, /, dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE5].startX + 1, 		pml, delta.x );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE5].startY + 1, 		pml, delta.y );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE5].startX + 1, 		pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE5].startY + 1, 		pml, delta.y );
		VelocityPMLUpdate( EDGE5, dampingMinusX, * , dampingMinusY, * ,  ,  , /, dampingPlusX, /, dampingPlusY,  ,  );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE6].startX + 1, 		pml, delta.x );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE6].endZ - iz, 				pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE6].startX + 1, 		pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE6].endZ - iz, 				pml, delta.z );
		VelocityPMLUpdate( EDGE6, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE7].startX + 1, 		pml, delta.x );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE7].startZ + 1, 		pml, delta.z );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE7].startX + 1, 		pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE7].startZ + 1, 		pml, delta.z );
		VelocityPMLUpdate( EDGE7, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endY - iy, 				pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endZ - iz, 				pml, delta.z );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endY - iy, 				pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endZ - iz, 				pml, delta.z );
		VelocityPMLUpdate( EDGE8,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE9].endY - iy, 				pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE9].startZ + 1, 		pml, delta.z );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE9].endY - iy, 				pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE9].startZ + 1, 		pml, delta.z );
		VelocityPMLUpdate( EDGE9,  ,  , dampingMinusY, * , dampingMinusZ, * ,  , , /, dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE10].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE10].endZ - iz, 			pml, delta.z );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE10].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE10].endZ - iz, 			pml, delta.z );
		VelocityPMLUpdate( EDGE10,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /,  dampingPlusY, /, dampingPlusZ );
	}
	...
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE11].startY + 1, 		pml, delta.y );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE11].startZ + 1, 		pml, delta.z );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE11].startY + 1, 		pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE11].startZ + 1, 		pml, delta.z );
		VelocityPMLUpdate( EDGE11,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /, dampingPlusY, /, dampingPlusZ );
	}
}
 ...
 __global__ void stressUpdate( GRID grid, Delta delta, VelocityStress vsGPU, DifferenceCoefficient dc, Medium medGPU, int pml )//__global__ 
{
	...
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX0].startX, vsPML[VERTEX0].startY, vsPML[VERTEX0].startZ, vsPML[VERTEX0].endX, vsPML[VERTEX0].endY, vsPML[VERTEX0].endZ ) ) 
	{
		...
		StressPMLUpdate(  VERTEX0, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , / , dampingPlusX, / , dampingPlusY, / , dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX1].startX, vsPML[VERTEX1].startY, vsPML[VERTEX1].startZ, vsPML[VERTEX1].endX, vsPML[VERTEX1].endY, vsPML[VERTEX1].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate(  VERTEX1, dampingMinusX, * , dampingMinusY, * , dampingMinusZ,  * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX2].startX, vsPML[VERTEX2].startY, vsPML[VERTEX2].startZ, vsPML[VERTEX2].endX, vsPML[VERTEX2].endY, vsPML[VERTEX2].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX2, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX3].startX, vsPML[VERTEX3].startY, vsPML[VERTEX3].startZ, vsPML[VERTEX3].endX, vsPML[VERTEX3].endY, vsPML[VERTEX3].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX3, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX4].startX, vsPML[VERTEX4].startY, vsPML[VERTEX4].startZ, vsPML[VERTEX4].endX, vsPML[VERTEX4].endY, vsPML[VERTEX4].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX4, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX5].startX, vsPML[VERTEX5].startY, vsPML[VERTEX5].startZ, vsPML[VERTEX5].endX, vsPML[VERTEX5].endY, vsPML[VERTEX5].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX5, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX6].startX, vsPML[VERTEX6].startY, vsPML[VERTEX6].startZ, vsPML[VERTEX6].endX, vsPML[VERTEX6].endY, vsPML[VERTEX6].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX6, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	else if( AreaCondition( ix, iy, iz, vsPML[VERTEX7].startX, vsPML[VERTEX7].startY, vsPML[VERTEX7].startZ, vsPML[VERTEX7].endX, vsPML[VERTEX7].endY, vsPML[VERTEX7].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		StressPMLUpdate( VERTEX7, dampingMinusX, * , dampingMinusY, * , dampingMinusZ, * , /, dampingPlusX, /, dampingPlusY, /, dampingPlusZ ) 
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE0].startX, vsPML[SURFACE0].startY, vsPML[SURFACE0].startZ, vsPML[SURFACE0].endX, vsPML[SURFACE0].endY, vsPML[SURFACE0].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE0].endX - ix, pml, delta.x );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE0].endX - ix, pml, delta.x );
		StressPMLUpdate( SURFACE0, dampingMinusX, *,  ,  ,  ,  , /, dampingPlusX,  ,  ,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE1].startX, vsPML[SURFACE1].startY, vsPML[SURFACE1].startZ, vsPML[SURFACE1].endX, vsPML[SURFACE1].endY, vsPML[SURFACE1].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusX = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[SURFACE1].startX + 1, pml, delta.x );
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[SURFACE1].startX + 1, pml, delta.x );
		StressPMLUpdate( SURFACE1, dampingMinusX, * ,  ,  ,  ,  , / , dampingPlusX,  ,  ,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE2].startX, vsPML[SURFACE2].startY, vsPML[SURFACE2].startZ, vsPML[SURFACE2].endX, vsPML[SURFACE2].endY, vsPML[SURFACE2].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE2].endY - iy, pml, delta.y );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE2].endY - iy, pml, delta.y );
		StressPMLUpdate( SURFACE2,  ,  , dampingMinusY, * ,  ,  ,  ,  , / , dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE3].startX, vsPML[SURFACE3].startY, vsPML[SURFACE3].startZ, vsPML[SURFACE3].endX, vsPML[SURFACE3].endY, vsPML[SURFACE3].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusY = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[SURFACE3].startY + 1, pml, delta.y );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[SURFACE3].startY + 1, pml, delta.y );
		StressPMLUpdate( SURFACE3,  ,  , dampingMinusY,  * ,  ,  ,  ,  , /, dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE4].startX, vsPML[SURFACE4].startY, vsPML[SURFACE4].startZ, vsPML[SURFACE4].endX, vsPML[SURFACE4].endY, vsPML[SURFACE4].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE4].endZ - iz, pml, delta.z );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[SURFACE4].endZ - iz, pml, delta.z );
		StressPMLUpdate( SURFACE4,  ,  ,  ,  , dampingMinusZ,  * ,  ,  ,  ,  , /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[SURFACE5].startX, vsPML[SURFACE5].startY, vsPML[SURFACE5].startZ, vsPML[SURFACE5].endX, vsPML[SURFACE5].endY, vsPML[SURFACE5].endZ ) )
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		float dampingMinusZ = 1.0f - 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[SURFACE5].startZ + 1, pml, delta.z );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[SURFACE5].startZ + 1, pml, delta.z );
		StressPMLUpdate( SURFACE5,  ,  ,  ,  , dampingMinusZ, * ,  ,  ,  ,  , /,  dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE0].startX, vsPML[EDGE0].startY, vsPML[EDGE0].startZ, vsPML[EDGE0].endX, vsPML[EDGE0].endY, vsPML[EDGE0].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endX - ix, pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE0].endY - iy, pml, delta.y );
		StressPMLUpdate( EDGE0, dampingMinusX, * , dampingMinusY,  * ,  ,  , /, dampingPlusX, /,  dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE1].startX, vsPML[EDGE1].startY, vsPML[EDGE1].startZ, vsPML[EDGE1].endX, vsPML[EDGE1].endY, vsPML[EDGE1].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE1].endX - ix, 			pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE1].startY + 1, 	pml, delta.y );
		StressPMLUpdate( EDGE1, dampingMinusX, * , dampingMinusY, * ,  ,  , /,  dampingPlusX, /,  dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE2].startX, vsPML[EDGE2].startY, vsPML[EDGE2].startZ, vsPML[EDGE2].endX, vsPML[EDGE2].endY, vsPML[EDGE2].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endX - ix, pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE2].endZ - iz, pml, delta.z );
		StressPMLUpdate( EDGE2, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , / , dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE3].startX, vsPML[EDGE3].startY, vsPML[EDGE3].startZ, vsPML[EDGE3].endX, vsPML[EDGE3].endY, vsPML[EDGE3].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE3].endX - ix, 			pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE3].startZ + 1, 	pml, delta.z );
		StressPMLUpdate( EDGE3, dampingMinusX,  * ,  ,  , dampingMinusZ,  * , /,  dampingPlusX,  ,  , / ,  dampingPlusZ )
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE4].startX, vsPML[EDGE4].startY, vsPML[EDGE4].startZ, vsPML[EDGE4].endX, vsPML[EDGE4].endY, vsPML[EDGE4].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE4].startX + 1, 	pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE4].endY - iy, 			pml, delta.y );
		StressPMLUpdate( EDGE4, dampingMinusX, * , dampingMinusY, * ,  ,  , /,  dampingPlusX, /, dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE5].startX, vsPML[EDGE5].startY, vsPML[EDGE5].startZ, vsPML[EDGE5].endX, vsPML[EDGE5].endY, vsPML[EDGE5].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE5].startX + 1, 	pml, delta.x );
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE5].startY + 1, 	pml, delta.y );
		StressPMLUpdate( EDGE5, dampingMinusX, * , dampingMinusY, * ,  ,  , /, dampingPlusX, /, dampingPlusY,  ,  );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE6].startX, vsPML[EDGE6].startY, vsPML[EDGE6].startZ, vsPML[EDGE6].endX, vsPML[EDGE6].endY, vsPML[EDGE6].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE6].startX + 1, 	pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE6].endZ - iz, 			pml, delta.z );
		StressPMLUpdate( EDGE6, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE7].startX, vsPML[EDGE7].startY, vsPML[EDGE7].startZ, vsPML[EDGE7].endX, vsPML[EDGE7].endY, vsPML[EDGE7].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusX  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, ix - vsPML[EDGE7].startX + 1, 	pml, delta.x );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE7].startZ + 1, 	pml, delta.z );
		StressPMLUpdate( EDGE7, dampingMinusX, * ,  ,  , dampingMinusZ, * , /, dampingPlusX,  ,  , /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE8].startX, vsPML[EDGE8].startY, vsPML[EDGE8].startZ, vsPML[EDGE8].endX, vsPML[EDGE8].endY, vsPML[EDGE8].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endY - iy, pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE8].endZ - iz, pml, delta.z );
		StressPMLUpdate( EDGE8,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE9].startX, vsPML[EDGE9].startY, vsPML[EDGE9].startZ, vsPML[EDGE9].endX, vsPML[EDGE9].endY, vsPML[EDGE9].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE9].endY - iy, 			pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE9].startZ + 1, 	pml, delta.z );
		StressPMLUpdate( EDGE9,  ,  , dampingMinusY, * , dampingMinusZ, * ,  , , /, dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE10].startX, vsPML[EDGE10].startY, vsPML[EDGE10].startZ, vsPML[EDGE10].endX, vsPML[EDGE10].endY, vsPML[EDGE10].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE10].startY + 1, 	pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, vsPML[EDGE10].endZ - iz, 		pml, delta.z );
		StressPMLUpdate( EDGE10,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /,  dampingPlusY, /, dampingPlusZ );
	}
	else if( AreaCondition( ix, iy, iz, vsPML[EDGE11].startX, vsPML[EDGE11].startY, vsPML[EDGE11].startZ, vsPML[EDGE11].endX, vsPML[EDGE11].endY, vsPML[EDGE11].endZ ) ) 
	{
		float Vs_wave = sqrt( medGPU.mu[INDEX(ix, iy, iz)] * medGPU.buoyancy[INDEX(ix, iy, iz)] );
		...
		float dampingPlusY  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iy - vsPML[EDGE11].startY + 1, pml, delta.y );
		float dampingPlusZ  = 1.0f + 0.5f * delta.t * DAMPING( Vs_wave, iz - vsPML[EDGE11].startZ + 1, pml, delta.z );
		StressPMLUpdate( EDGE11,  ,  , dampingMinusY, * , dampingMinusZ, * ,  ,  , /, dampingPlusY, /, dampingPlusZ );
	}
}
